"""
Calculate validation MAE for the EfficientNetV2-M model.
"""
...
def main():
    print("=" * 70)
    print("Calculate Validation MAE - EfficientNetV2-M")
    print("=" * 70)
    ...
    for name in TARGET_NAMES:
        preds = all_predictions_denorm[name]
        tgts = all_targets_denorm[name]
        mae = sum(abs(p - t) for p, t in zip(preds, tgts)) / len(preds)
        total_mae += mae * len(preds)
        count += len(preds)
        print(f"{name:<20} MAE: {mae:.4f} (n={len(preds)})")

    overall_mae = total_mae / count
    print(f"\n{'Overall MAE':<20} {overall_mae:.4f}")

    print("\n" + "=" * 70)
    print(f"KAGGLE PUBLIC SCORE:  0.42 MAE")
    print(f"VALIDATION MAE:       {overall_mae:.4f}")
    print(f"GAP:                  {abs(0.42 - overall_mae):.4f}")
    print("=" * 70)
